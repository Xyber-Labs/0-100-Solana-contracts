...
describe("Raydium CLMM Pool Creation - Fast Flow", () => {
  ...
  //   const buyerAmounts = [
  //     Number(process.env.BUYER1_AMOUNT || "100"),
  //     Number(process.env.BUYER2_AMOUNT || "100"),
  //     Number(process.env.BUYER3_AMOUNT || "100"),
  //   ];
  //   const creatorDepositSol = 8;
  //   const totalDeposited = buyerAmounts.reduce((a, b) => a + b, 0) + creatorDepositSol;
  //
  ...
  //
  //     const expectedShare = saleAllocation
  //       .mul(new BN(buyerAmounts[i]))
  //       .div(new BN(totalDeposited));
  //
  //     const diff = balance.sub(expectedShare).abs();
  //     console.log(`Buyer ${i + 1}: claimed ${balance.toString()}, expected ${expectedShare.toString()}, diff ${diff.toString()}`);
  //
  //     assert.ok(diff.lte(new BN(1000)), `Buyer ${i + 1}: diff ${diff.toString()} exceeds tolerance`);
  //   }
  //
  ...
  //
  //   const creatorDepositSol = 8;
  //   const totalDepositedSol = 308;
  //   const creatorSaleShare = saleAllocation.mul(new BN(creatorDepositSol)).div(new BN(totalDepositedSol));
  //   const salePeriods = creatorDepositSol;
  //   const teamPeriods = preset.teamDurationSec / preset.teamPeriodSec;
  //
  //   console.log(`Sale allocation: ${saleAllocation.toString()}`);
  //   console.log(`Creator sale share: ${creatorSaleShare.toString()}`);
  //   console.log(`Sale periods: ${salePeriods}, per period: ${creatorSaleShare.div(new BN(salePeriods)).toString()}`);
  ...
  //
  //     const expectedSalePerPeriod = creatorSaleShare.div(new BN(salePeriods));
  //     const expectedTeamPerPeriod = teamAllocation.div(new BN(teamPeriods));
  //
  //     console.log(`  Sale: +${saleGrowth.toString()}, expected/period: ${expectedSalePerPeriod.toString()}`);
  ...
  //   await new Promise(resolve => setTimeout(resolve, (salePeriods - periodsToTest) * periodInterval + 2000));
  //
  //   await sdk.claim({ launch: launchPda, baseMint, participantKeypair: creatorKeypair, bucket: 0 });
  ...
  //   assert.ok(saleDiff.lte(new BN(100)), `Total sale claimed should equal creator's sale share (diff: ${saleDiff.toString()})`);
  //
