...
spl-token create-token \
  --url localhost \
  --fee-payer devnet/multisig.json \
  --mint-authority devnet/multisig.json \
  --decimals 6 \
  devnet/xyber-mint.json

# Create token account for creator
spl-token create-account $XYBER_MINT \
  --owner $CREATOR \
  --url devnet \
  --fee-payer devnet/creator.json

# Create token account for treasury
export TREASURY=$(solana address -k devnet/treasure.json)
spl-token create-account $XYBER_MINT \
  --owner $TREASURY \
  --url devnet \
  --fee-payer devnet/multisig.json
...
spl-token mint --url devnet --recipient-owner $CREATOR --mint-authority devnet/multisig.json $XYBER_MINT 1000000000
```
...
```bash
anchor run close-clmm-position --provider.cluster ${CLUSTER} -- \
  --project-id ${PROJECT_ID} \
  --creator-keypair ${CLUSTER}/creator.json
```
