...
import { createAndFundAccount } from "../tests/utils";
...
describe("engine litesvm - raydium clmm", () => {
  ...
  it("Adds liquidity to CLMM pool", async () => {
    ...
    const RENT_RESERVE = new anchor.BN(200_000_000);
    const quoteAmountForLiquidity = quoteAmountLamports.sub(RENT_RESERVE);

    console.log(`Base amount: ${baseAmount.toString()} (${Number(baseAmount) / 1e9} tokens)`);
    console.log(`Quote amount (total deposited): ${quoteAmountLamports.toString()} lamports (${quoteAmountLamports.div(new anchor.BN(anchor.web3.LAMPORTS_PER_SOL)).toString()} SOL)`);
    console.log(`Quote amount (for liquidity): ${quoteAmountForLiquidity.toString()} lamports (${quoteAmountForLiquidity.div(new anchor.BN(anchor.web3.LAMPORTS_PER_SOL)).toString()} SOL)`);
    ...
    console.log(`Sale tokens: ${TOKENS_SOLD_ON_SALE}M tokens`);
    ...
    let txResult;
    try {
      txResult = client.sendTransaction(addLiquidityResultTx.transaction);
    } catch (err) {
      console.log("\n❌ Transaction threw error during sendTransaction");
      console.log("Error:", err);
      throw err;
    }

    console.log("\n=== Transaction Result ===");
    console.log("Transaction result keys:", Object.keys(txResult));
    console.log("Error present:", !!txResult.err);

    if (txResult.err) {
      console.log("\n❌ Transaction failed");
      const errorDetails = typeof txResult.err === 'function' ? txResult.err() : txResult.err;
      console.log("Error details:", errorDetails);
      console.log("Full tx result:", JSON.stringify(txResult, null, 2));

      console.log("\n=== Transaction Logs ===");
      const logs = (txResult as any).meta?.logMessages || (txResult as any).logs || [];
      logs.forEach((log: string, i: number) => {
        console.log(`Log ${i}: ${log}`);
      });

      throw new Error(`Transaction failed with error: ${JSON.stringify(errorDetails)}`);
    }

    console.log("\n✅ Liquidity transaction sent");

    console.log("\n=== Checking Raydium Pool Vaults ===");
    const quoteVaultAccount = client.getAccount(addLiquidityResultTx.quoteVault);
