...
fn get_liquidity_range_impl(tick_spacing: u16, price_ratio: f64, straight: bool) -> LiquidityRange {
    ...
    let sqrt_price_middle_x64 = (price.sqrt() * fixed_point_64::Q64 as f64) as u128;
    ...
    let tick_middle_raw = tick_math::get_tick_at_sqrt_price(sqrt_price_middle_x64).unwrap();

    let spacing = tick_spacing as i32;
    let tick_array_lower = (tick_lower_raw / spacing) * spacing;
    let tick_array_upper = ((tick_upper_raw + spacing - 1) / spacing) * spacing;
    let tick_array_middle = (tick_middle_raw / spacing) * spacing;
    ...
    let tick_array_middle_start_index =
        TickArrayState::get_array_start_index(tick_middle_raw, tick_spacing);
