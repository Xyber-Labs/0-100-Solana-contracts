...
pub fn creator_deposit(ctx: Context<CreatorDeposit>, amount: u64) -> Result<()> {
    ...
    let new_locked =
        grant.locked_lamports.checked_add(amount).ok_or(EngineErrorCode::ArithmeticOverflow)?;
    ...
    state.total_deposited =
        state.total_deposited.checked_add(amount).ok_or(EngineErrorCode::ArithmeticOverflow)?;
