...
mod tests {
    ...
    use crate::utils::lottery::{Lottery, LotteryStatus};
    ...
    #[test]
    fn test_get_bit() {
        let lottery = Lottery { bits_allocated: 64, inactive: 0, status: LotteryStatus::InProgress, bits: vec![0b1010] };
    ...
    fn test_get_bit_second_word() {
        let lottery = Lottery { bits_allocated: 128, inactive: 0, status: LotteryStatus::InProgress, bits: vec![0, 0b101] };
    ...
    fn test_set_bit_simple() {
        let mut lottery = Lottery { bits_allocated: 128, inactive: 0, status: LotteryStatus::InProgress, bits: vec![0; 2] };
    ...
    fn test_set_bit_collision() {
        let mut lottery = Lottery { bits_allocated: 64, inactive: 0, status: LotteryStatus::InProgress, bits: vec![0b111] };
        assert_eq!(lottery.set_bit(0, &[]), Some(3));
    }

    #[test]
    fn test_set_bit_collision_cross_word() {
        let mut lottery = Lottery { bits_allocated: 128, inactive: 0, status: LotteryStatus::InProgress, bits: vec![u64::MAX, 0] };
        assert_eq!(lottery.set_bit(60, &[]), Some(64));
    }

    #[test]
    fn test_set_bit_wraparound() {
        let mut lottery = Lottery { bits_allocated: 64, inactive: 0, status: LotteryStatus::InProgress, bits: vec![!0b11] };
        assert_eq!(lottery.set_bit(60, &[]), Some(0));
    }

    #[test]
    fn test_set_bit_wraparound_cross_word() {
        let mut lottery = Lottery { bits_allocated: 128, inactive: 0, status: LotteryStatus::InProgress, bits: vec![0b1, u64::MAX] };
        assert_eq!(lottery.set_bit(100, &[]), Some(1));
    }

    #[test]
    fn test_set_bit_full() {
        let mut lottery = Lottery { bits_allocated: 64, inactive: 0, status: LotteryStatus::InProgress, bits: vec![u64::MAX] };
        assert_eq!(lottery.set_bit(0, &[]), None);
    }

    #[test]
    fn test_set_bit_out_of_bounds() {
        let mut lottery = Lottery { bits_allocated: 64, inactive: 0, status: LotteryStatus::InProgress, bits: vec![0] };
    ...
    fn test_set_bit_skips_withdrawn_wraparound() {
        let mut lottery = Lottery { bits_allocated: 64, inactive: 0, status: LotteryStatus::InProgress, bits: vec![0b1111] };
    ...
    fn test_count_ones_empty() {
        let lottery = Lottery { bits_allocated: 128, inactive: 0, status: LotteryStatus::InProgress, bits: vec![0; 2] };
        assert_eq!(lottery.count_ones(&TicketRange::new(0, 128)), 0);
    }

    #[test]
    fn test_count_ones_all_set() {
        let lottery = Lottery { bits_allocated: 64, inactive: 0, status: LotteryStatus::InProgress, bits: vec![u64::MAX] };
        assert_eq!(lottery.count_ones(&TicketRange::new(0, 64)), 64);
    }

    #[test]
    fn test_count_ones_partial() {
        let lottery = Lottery { bits_allocated: 64, inactive: 0, status: LotteryStatus::InProgress, bits: vec![0b11111] };
    ...
    fn test_count_ones_cross_word() {
        let lottery = Lottery { bits_allocated: 128, inactive: 0, status: LotteryStatus::InProgress, bits: vec![u64::MAX, 0b1111] };
    ...
    fn test_allocate_overflow() {
        let mut lottery = Lottery { bits_allocated: u64::MAX - 5, inactive: 0, status: LotteryStatus::InProgress, bits: vec![] };
    ...
    fn test_finalize_no_overflow() {
        let seed = [1u8; 32];
        let mut lottery = Lottery { bits_allocated: 64, inactive: 0, status: LotteryStatus::InProgress, bits: vec![0] };
        let winners = lottery.finalize(&seed, 100, &[], 6400, 64000).unwrap();
        assert_eq!(winners, 64);
        assert_eq!(lottery.count_ones(&TicketRange::new(0, 64)), 64);
        assert_eq!(lottery.status, LotteryStatus::Finalized { tokens_per_ticket: 1000, sol_per_ticket: 100 });
    }
    ...
    fn test_finalize_overflow() {
        let seed = [2u8; 32];
        let mut lottery = Lottery { bits_allocated: 128, inactive: 0, status: LotteryStatus::InProgress, bits: vec![0; 2] };
        let winners = lottery.finalize(&seed, 50, &[], 5000, 50000).unwrap();
    ...
    fn test_finalize_zero_tickets_panics() {
        let seed = [3u8; 32];
        let mut lottery = Lottery { bits_allocated: 0, inactive: 0, status: LotteryStatus::InProgress, bits: vec![] };
        let _ = lottery.finalize(&seed, 50, &[], 0, 0);
    }

    #[test]
    fn test_finalize_with_withdrawn() {
        let seed = [4u8; 32];
        let mut lottery = Lottery { bits_allocated: 100, inactive: 20, status: LotteryStatus::InProgress, bits: vec![0; 2] };
        let withdrawn = vec![TicketRange::new(10, 30)];
        let winners = lottery.finalize(&seed, 50, &withdrawn, 5000, 50000).unwrap();
        assert_eq!(winners, 50);
        for i in 10..30 {
    ...
    fn test_finalize_with_withdrawn_all_win() {
        ...
        let winners = lottery.finalize(&seed, 100, &withdrawn, 8000, 80000).unwrap();
    ...
    fn test_count_winning_in_ranges_empty() {
        let lottery = Lottery { bits_allocated: 64, inactive: 0, status: LotteryStatus::InProgress, bits: vec![0] };
    ...
    fn test_count_winning_in_ranges_partial() {
        let lottery = Lottery { bits_allocated: 64, inactive: 0, status: LotteryStatus::InProgress, bits: vec![0b11111111] };
        let ranges = vec![
            TicketRange::new(0, 10),
            TicketRange::new(20, 25),
        ];
