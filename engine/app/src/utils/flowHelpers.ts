  ...
  await runWithConcurrency(users, Math.min(concurrency, total), async (user, index) => {
    ...
    try {
      const res = await (async () => {
        for (;;) {
          try {
            return await (sdk as any).depositAutoShard({ launch: launchPda, amountLamports: user.depositAmount, userKeypair: user.keypair, preferredShardId: user.shardId });
          } catch (e: any) {
            const msg = String(e?.message || "");
            const transient = msg.includes("aborted") || msg.includes("Blockhash") || msg.includes("429") || msg.includes("Too many") || msg.includes("ETIMEDOUT") || msg.includes("ECONNRESET") || msg.includes("not confirmed in 30.00 seconds");
            attempt++;
            if (!transient || attempt >= maxAttempts) throw e;
